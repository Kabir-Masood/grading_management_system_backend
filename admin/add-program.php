...
if (!empty($_POST)) {
    ...
    $program_code = htmlentities($_POST['program_code'], ENT_QUOTES);
    $started_on = htmlentities($_POST['started_on'], ENT_QUOTES);

    if (!empty($name) && !empty($program_code) && !empty($started_on)) {
        $created_date = date("Y-m-d h:i:sa", time());
        $mProgram->save_program($name, $program_code, $started_on, "Admin", $created_date, "", null, null, 1);
...
//echo $rowData[0][0][1];
            ...
            foreach ($rowData as $datum) {
                foreach ($datum as $item) {
                    ...
                    $mProgram->save_program($program_name, $program_code, $started_on, "Admin", $created_date, "null", "null", "null", 1);
                }
            }
                    ...
                    <div class="table-responsive">
                        ...
                        <form action="add-program.php" method="post">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="name" placeholder="Enter program name"
                                       name="name" required>
                            </div>
                            <div class="form-group">
                                <label for="text">Program Code:</label>
                                <input type="text" class="form-control" id="program_code" placeholder="Enter program code"
                                       name="program_code" required>
                            </div>
                            <div class="form-group">
                                <label for="text">Started On:</label>
                                <input type="date" class="form-control" id="started_on" name="started_on" required>
                            </div>
                        ...
                        <br><br>
                        <h2>Upload program list from xlsx spreadsheet:</h2>
                        ...
                        <p style="color: #05a4cf">[N.B]: File must be in .xlsx format and should contain these field(program_name,program_code,started_on)</p>
                    </div>
                </div>

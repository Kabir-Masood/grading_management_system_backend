...
if (!empty($_POST)) {
    ...
    if (!empty($course_name) && !empty($course_code) && !empty($course_credit) && $dept_id !== 0 && $teacher_id !== 0) {
        $created_date = date("Y-m-d h:i:sa", time());
        $mCourse->save_course($course_name, $course_code, $course_credit, $teacher_id, 0, $dept_id, "Admin", $created_date, "", null, "", 1);
...
//echo $rowData[0][0][1];
            ...
            foreach ($rowData as $datum) {
                foreach ($datum as $item) {
                    ...
                    $course_name = $item[0];
                    $course_code = $item[1];
                    $course_credit = $item[2];
                    $teacher_id = $item[3];
                    $semester_id = $item[4];
                    $dept_id = $item[5];
                    $created_date = date("Y-m-d h:i:sa", time());

                    $mCourse->save_course($course_name, $course_code, $course_credit, $teacher_id, $semester_id, $dept_id, "Admin", $created_date, "null", "null", "null",1);
                }
            }
                    ...
                    <div class="table-responsive">
                        ...
                        <form action="add-course.php" method="post">
                            ...
                            <div class="form-group">
                                ...
                                <input type="double" class="form-control" id="course_credit"
                        ...
                        <br>
                        <p style="color: #33abd8;">[N.B] File must be xlsx format. (id, course_name, course_code, course_credit, teacher_id, semester_id, dept_id)</p>
                    </div>
                </div>
                <div class="card-footer small text-muted">Updated yesterday at 11:59 PM</div>
            </div>

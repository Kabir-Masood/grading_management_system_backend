...
if (!empty($_POST)) {
    ...
    if (!empty($user_id) && !empty($user_name) && !empty($email) &&
        !empty($user_phone) && !empty($user_password) && $dept != 0) {
        $created_date = date("Y-m-d h:i:sa", time());
        $mTeacher->save_teacher($user_name, $email, $user_id, $user_password, $user_phone, $dept, "Admin", $created_date, null, null, null, 1);
...
//echo $rowData[0][0][1];
            ...
            foreach ($rowData as $datum) {
                foreach ($datum as $item) {
                    ...
                    $mTeacher->save_teacher($full_name, $email, $user_id, $password, $mobile, $dept_id, "Admin", $created_date, "null", "null", "null", 1);
                }
            }
                    ...
                    <div class="table-responsive">
                        ...
                        <form action="add-teacher.php" method="post" autocomplete="off">
                            <div class="form-group">
                                <label for="number">Teacher ID:</label>
                                <input type="number" class="form-control" id="user_id" placeholder="Enter teacher ID"
                                       name="user_id">
                            </div>
                            <div class="form-group">
                        ...
                        <h2>Upload teacher list from xlsx spreadsheet:</h2>
                        ...
                        <p style="color: #05a4cf">[N.B]: File must be in .xlsx format and should contain these
                            field(full_name,email,user_id,password,mobile,dept_id[FK])</p>
                    </div>
                </div>
                <div class="card-footer small text-muted">Updated yesterday at 11:59 PM</div>
            </div>
